...
def train():
    ...
    for fold, (train_idx, val_idx) in enumerate(splits.split(np.arange(len(DATASET)))):
        ...
        for epoch in range(0, args.epochs):
            ...
            print(train_loss)
            print(test_stats)
            writer.add_scalar('train_loss_fold ' + str(fold + 1), train_loss, EPOCH_CNT)
            writer.add_scalars('val_fold' + str(fold + 1), {key: test_stats[key] for key in test_stats},
                               EPOCH_CNT)
            # if test_stats['precision'] > prec_ls[0, fold]:
                # prec_ls[0, fold] = test_stats['precision']
            if test_stats['mAP_disease'] > mAP_ls[0, fold]:
                mAP_ls[0, fold] = test_stats['mAP_disease']
            if test_stats['mAP_disease_organ'] > mAP_ls[1, fold]:
                mAP_ls[1, fold] = test_stats['mAP_disease_organ']
            if test_stats['mAP_disease_organ_location'] > mAP_ls[2, fold]:
                mAP_ls[2, fold] = test_stats['mAP_disease_organ_location']
        # print('fold_performance: {}'.format(prec_ls[0, fold]))
        print('fold_performance: {}'.format(mAP_ls[:, fold]))
    # after training for all folds
    # print(prec_ls)
    print(mAP_ls)
    # print('avg_preformance {}'.format(np.mean(prec_ls)))
    print('avg_preformance {}'.format(np.mean(mAP_ls, axis=1)))
